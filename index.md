...
[`extract_schema()`](reference/extract_schema.md),
[`gen_alternatives()`](reference/gen_alternatives.md),
[`gen_code()`](reference/gen_code.md), and
[`validate_script()`](reference/validate_script.md) all call out to an
LLM via [ellmer](https://ellmer.tidyverse.org/). Set the relevant API
key as an environment variable before using them, e.g. for Claude
models:
...
for the full list of supported providers and their expected environment
variables.
...
attempt, [`validate_script()`](reference/validate_script.md) can be used
to iteratively fix it until it runs:
...
You can load an alternative YAML file with
[`read_alternatives()`](reference/read-write-alternatives.md):
