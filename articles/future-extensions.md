...
## A single-file multiverse

[`expand_tines()`](../reference/expand.md) returns a `multiverse` – a
collection of schemas – and
[`write_tines()`](../reference/read-write.md) can persist one. Whether
it *should* is unsettled.

A multiverse produced by expansion is fully derived from its inputs: the
same base schema and alternatives file reproduce it exactly. Persisting
it is therefore convenience rather than necessity, and the expanded form
is both bulkier than its inputs (one near-complete copy of the schema
per branch) and less informative (the base-versus-varied structure has
to be recovered by diffing branches).

The cases where a multiverse is *not* re-derivable are the ones that
would justify a format:

- branches pruned after expansion, because a cross-product generates
  combinations that are not all coherent;
- a branch patched by hand with a step no alternative expresses;
- a chain of expansions, where nothing records the order they were
  applied in;
- a collection of independently authored analyses, as in a many-analysts
  study, which no expansion produced.

None of those are supported workflows today, and a results-carrying
variant – mapping each branch to its script and its estimate – would be
a different artifact again, closer to a manifest than to a schema
collection. Designing the format before deciding which of these matters
would mean designing the wrong one.
