...
#' )
#'
extract_schema <- function(text, data_dict, output_file = "draft_schema.yml",
  ...
  data_summary <- if (is.data.frame(data_dict)) {
    if (!"name" %in% names(data_dict)) {
      cli::cli_abort(
        "{.arg data_dict} data frame must have at least a {.field name} column."
      )
    }
    ...
    cli::cli_abort(
      "{.arg data_dict} must be a character vector or a data frame."
    )
  }
  ...
  cli::cli_alert_info(
    "Extracting schema and mapping data flow simultaneously..."
  )
  ...
  cli::cli_alert_success(
    "DAG schema drafted and mapped successfully to {.file {output_file}}"
  )
...
prompt_extract_schema <- function(data_dict, text, print = TRUE, width = 70) {
  system_prompt <- paste0(
    "You are an expert methodologist and data pipeline architect. ",
    "I will provide a text describing ",
    "a multiverse analysis and a summary of the actual dataset being used.\n\n",
    "=== YOUR TASK === \n\n Extract a chronological list of methodological ",
    "decisions (nodes) AND map the ",
    ...
    "the 'objective'.\n\n",
    "   - 'rationale': WHY that decision was made, extracted from ",
    "the text.\n\n",
    "2. DATA MAPPING: Assign 'inputs' (EXACT column names from the dataset ",
    "OR outputs from previous nodes) ",
    "and 'outputs' (invented snake_case objects like 'df_clean' or ",
    "'ranef_spec').\n\n",
    "3. ANTI-ABSTRACTION (CRITICAL): If the text lists specific variables ",
    "(e.g., 'centered rater, meanIAT'), ",
    "DO NOT summarize them away. You MUST capture those specific variables ",
    "in the 'inputs' array.\n\n",
    "4. INLINE ARRAYS: Format arrays strictly on one line: ",
    "`inputs: [var1, var2]`.\n\n",
    "5. CONFIDENCE & CLARIFICATION: Rate your mapping confidence ",
    "(HIGH, MEDIUM, LOW). ",
    "If the text abstracts a step and you cannot confidently match it to ",
    "specific dataset columns, ",
    "set confidence to LOW and autogenerate a 'clarification_question' ",
    "asking the user which exact ",
